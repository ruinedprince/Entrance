...
export default function EventDetails() {
    ...
    const [userType, setUserType] = useState('participante'); // Example user type
    ...
    useEffect(() => {
        ...
        if (!id) {
            console.error('ID do evento não foi fornecido na URL.');
    ...
    return (
        <div>
                    ...
                    <tbody>
                        {tickets.map(ticket => (
                            <tr key={ticket.id}>
                                ...
                                <td>
                                    <button onClick={() => window.location.href = `/ticket_edit?id=${ticket.id}`}>Editar</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            {userType === 'participante' ? (
                <button onClick={() => router.push('/ticket_register')}>Reservar</button>
            ) : userType === 'admin' || userType === 'produtor' ? (
                <button onClick={() => router.push(`/event_edit?id=${id}`)}>Editar</button>
            ) : null}
        </div>
    );
}